...
from bnn_tools.funs_bokeh_flx import get_plot_flx_layout, CHC_LAT, CHC_LON
...
def bokeh_gr_gui(init_file, *,
    ...
    g_plot = gr_gui_plot_basic(_fun_open_df_top0, _fun_open_df_top1,
                               _fun_open_df_top2, _fun_open_df_top3,
                               _fun_open_df_top4,
                               _open_psd_ds, init_file, dic, d1, d2)
    ...
    bkp.show(bkapp, notebook_url=bfu.remote_jupyter_proxy_url)

def bokeh_gr_gui2(init_file, *,
                 _open_psd_ds,
                 _fun_open_df_top0,
                 _fun_open_df_top1,
                 _fun_open_df_top2,
    ...
    g_plot_ = gr_gui_plot_basic(_fun_open_df_top0, _fun_open_df_top1,
                               _fun_open_df_top2, _fun_open_df_top3,
                               _fun_open_df_top4,
                               _open_psd_ds, init_file, dic, d1, d2)
    ...
    ds = _fun_open_ds_flx(d1,d2)

    dd1 = d1.strftime('%Y-%m-%d %H')
    dd2 = d2.strftime('%Y-%m-%d %H')
    dd0 = d0.strftime('%Y-%m-%d %H')
    flx_col = get_plot_flx_layout(cen_lat=CHC_LAT,cen_lon=CHC_LON,d1=dd1,d2=dd2,ds = ds, d =dd0 )

    g_plot = bkl.row(flx_col,g_plot_)
...
def gr_gui_plot_basic(_fun_open_df_top0, _fun_open_df_top1, _fun_open_df_top2, _fun_open_df_top3, _fun_open_df_top4,
                      _open_psd_ds, init_file, dic, d1, d2):
    pdt_so = _open_source('poly_df', dic)
    mdt_so = _open_source('multi_df', dic)
    pnt_so = _open_source('point_df', dic)
    rec_so = _open_source('rec_df', dic)
    ...
    for fake_open_dfs in [1]:
        ...
        df_win_dir2 = _fun_open_df_top1(d1, d2)
        df_win_spd3 = _fun_open_df_top3(d1, d2)
        df_4 = _fun_open_df_top4(d1, d2)
        ds_psd = _open_psd_ds(d1, d2)
        df_bc1 = _fun_open_df_top2(d1, d2)
        df_rad0 = _fun_open_df_top0(d1, d2)
    # noinspection PyUnusedLocal
    for fake_cons1 in [1]:
        w = 700
        h = 350
    ...
    # noinspection PyUnusedLocal
    for fake_tools in [1]:
        p_to0.add_tools(p_ce.select_one({"name": "crosshair"}))
    ...
    for fake_set_pads in [1]:
        ...
        # p_to0.min_border_right = pad
    # noinspection PyUnusedLocal
    for fake_set_ranges in [1]:
        p_to0.x_range = p_ce.x_range
    ...
    for fake_save in [1]:
        ...
        # xy = np.array([])
        # noinspection PyUnusedLocal
        def _save_dic_when_sources_change(attr, old, new):
            # print('saving')
            save_dic(dic=dic,
                     srs_dic=srs,
        ...
        pdt_so.on_change('data', _save_dic_when_sources_change)
        mdt_so.on_change('data', _save_dic_when_sources_change)
        pnt_so.on_change('data', _save_dic_when_sources_change)
        rec_so.on_change('data', _save_dic_when_sources_change)
    ...
    c_top = bkl.column(p_to4, p_to3, p_to2, p_to1, p_to0)
    ...
    for fake_add_touchs in [1]:
        _px = "0px"
        p_to0.xaxis.major_label_text_font_size = _px
        p_to1.xaxis.major_label_text_font_size = _px
        p_to2.xaxis.major_label_text_font_size = _px
        p_to3.xaxis.major_label_text_font_size = _px
        p_to4.xaxis.major_label_text_font_size = _px
        p_bo.xaxis.major_label_text_font_size = _px

        # p_to0.yaxis.axis_label = 'rad'
